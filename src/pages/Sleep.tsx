...
export default function Sleep() {
  ...
  const loadSleepData = async () => {
    try {
      ...
      const { data: soundscapesData } = await supabase
        ...
        .select("*")
        .order("created_at", { ascending: false });
      if (soundscapesData) setSoundscapes(soundscapesData);
      ...
      if (user) {
        const { data: sleepData } = await supabase
          ...
          .limit(30);
        if (sleepData) setSleepData(sleepData);
      }
    } catch (error) {
  ...
  const calculateWeeklyAverage = () => {
    ...
    return (
      validData.reduce((sum, d) => sum + (d.sleep_quality || 0), 0) /
      validData.length
    );
  };
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <AppBar title="Sleep & Relaxation" showBack backTo="/dashboard" />
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Badge variant="outline">{soundscape.category}</Badge>
                      ...
                      <div className="flex items-center justify-between">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {entry.sleep_duration
                                ? `${Math.floor(entry.sleep_duration / 60)}h ${
                                    entry.sleep_duration % 60
                                  }m`
                        ...
                        <div className="text-right">
                          {entry.sleep_quality && (
                            <Badge
                              variant="outline"
                              className={getSleepQualityColor(
                                entry.sleep_quality
                              )}
                            >
